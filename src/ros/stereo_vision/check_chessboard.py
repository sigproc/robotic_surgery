...
for i in range(1,n):
    ...
    # read left images
    if i<10:
        image = cv2.imread('calibration_pictures/left_0'+repr(i)+'.ppm')
    else:
        image = cv2.imread('calibration_pictures/left_'+repr(i)+'.ppm')

    # convert images to grayscale      
    temp = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

    # identify and save chessboard's corners' coordinates
    ret1, corners1 = cv2.findChessboardCorners(temp,(9, 6))
...
for i in range(1,n):
    ...
    # read right images
    if i<10:
        image = cv2.imread('calibration_pictures/right_0'+repr(i)+'.ppm')
    else:
        image = cv2.imread('calibration_pictures/right_'+repr(i)+'.ppm')

    # convert images to grayscale            
    temp = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

    # identify and save chessboard's corners' coordinates
    ret2, corners2 = cv2.findChessboardCorners(temp,(9, 6))
...
np.save('calibration_data/left_image_points', left_image_points)
np.save('calibration_data/right_image_points', right_image_points)
