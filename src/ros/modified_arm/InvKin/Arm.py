...
class Arm3Link:
    ...
    def __init__(self, q=None, q0=None, L=None):
        ...
        if q is None: q = [math.pi/2, math.pi/4, math.pi/4]
        self.q = q
        # some default arm positions
        if q0 is None: q0 = np.array([math.pi/2, math.pi/4, math.pi/4]) 
        self.q0 = q0
        # arm segment lengths
        if L is None: L = np.array([1, 1, 1]) 
        self.L = L
        
        self.max_angles = [math.pi/2, math.pi, math.pi/4]
        self.min_angles = [math.pi/2, 0, -math.pi/4]
